...
from data_load import HeteroSyntheticDataset, collate
from CaseLink_model import HeteroCaseLink, early_stopping
...
def main():
    log_dir = get_path() + '/datasets/' + args.dataset + '/caselink_heterogeneous_experiments/'
    ...
    metadata = (['charge', 'case'], [('case', 'to', 'case'), ('charge', 'to', 'case'), ('charge', 'to', 'charge'), ('case', 'rev_to', 'case'), ('case', 'rev_to', 'charge'), ('charge', 'rev_to', 'charge')])
    # metadata = (['charge', 'case'], [('case', 'to', 'case'), ('charge', 'to', 'case'), ('charge', 'to', 'charge'), ('case', 'rev_to', 'charge')])
    model = HeteroCaseLink(hidden_channels=args.h_dim, dropout=args.dropout, metadata=metadata, num_layers=args.layer_num, num_attention_heads=args.num_heads)
    model.to(device)
    ...
    ## Load datasets
    ## Train dataset
    train_graph_and_label = torch.load(get_path() + '/datasets/' + args.dataset + '/graphs/caselink_heterogeneous/train_graph_bm25top' + str(args.topk_neighbor) + '_charge_thres' + str(args.charge_threshold) + '.pt')
    # del train_graph_and_label[('case', 'rev_to', 'case')]
    # del train_graph_and_label[('charge', 'rev_to', 'charge')]
    train_graphs = train_graph_and_label
    graph_labels = train_graph_and_label['case']['name_list'].tolist()
    train_label_list = [str(int(x)).zfill(6) for x in graph_labels]
    
    train_dataset = HeteroSyntheticDataset(file_path=get_path() + '/datasets/' + args.dataset + '/graphs/caselink_heterogeneous/train_graph_material_bm25top' + str(args.topk_neighbor) + '_charge_thres' + str(args.charge_threshold) + '.pt', label_dict=train_labels, train_pool=train_label_list, hard_neg_num=args.hard_neg_num, hard_bm25_dict=bm25_hard_neg_dict)
    ...
    train_dataloader = DataLoader(dataset=train_dataset, sampler=train_sampler, batch_size=args.batch_size, drop_last=False, collate_fn=collate)
    ...
    test_graph_and_label = torch.load(get_path() + '/datasets/' + args.dataset + '/graphs/caselink_heterogeneous/test_graph_bm25top' + str(args.topk_neighbor) + '_charge_thres' + str(args.charge_threshold) + '.pt')
    # del test_graph_and_label[('case', 'rev_to', 'case')]
    # del test_graph_and_label[('charge', 'rev_to', 'charge')]
    test_graphs = test_graph_and_label
    test_graph_labels = test_graph_and_label['case']['name_list'].tolist()
    ...
    for epoch in tqdm(range(args.epoch)):
        print('Epoch:', epoch)
        forward(args.dataset, model, device, writer, train_dataloader, train_graph_and_label, train_labels, train_candidate_list, yf_path, epoch, args.batch_size, args.temp, args.lamb, args.hard_neg_num, train_flag=True, test_mask=None, test_label_list=None, test_query_list=None, test_query_index_list=None, prediction_saving=False, optimizer=optimizer, training_setup=training_setup)
        with torch.no_grad():            
            ndcg_score_yf = forward(args.dataset, model, device, writer, test_dataloader, test_graph_and_label, test_labels, train_candidate_list, yf_path, epoch, args.batch_size, args.temp, args.lamb, args.hard_neg_num, train_flag=False, test_mask=test_mask, test_label_list=test_label_list, test_query_list=test_query_list, test_query_index_list=test_query_index_list, prediction_saving=False, optimizer=optimizer, training_setup=training_setup)

        if ndcg_score_yf > highest_ndcg_score_yf:
            highest_ndcg_score_yf = ndcg_score_yf
            ndcg_score_yf = forward(args.dataset, model, device, writer, test_dataloader, test_graph_and_label, test_labels, train_candidate_list, yf_path, epoch, args.batch_size, args.temp, args.lamb, args.hard_neg_num, train_flag=False, test_mask=test_mask, test_label_list=test_label_list, test_query_list=test_query_list, test_query_index_list=test_query_index_list, prediction_saving=True, optimizer=optimizer, training_setup=training_setup)
