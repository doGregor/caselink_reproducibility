...
def rank_2stage(final_pre_dict, top_10_list):   

    final_2stage_pre_dict = {}
    for i in final_pre_dict.keys():
        pred_list = final_pre_dict[i]
        
        ##2stage selcetion
        index_dict = {}
        for x in top_10_list[i][1:11]:
            index = pred_list.index(x)
            index_dict.update({x:index})
        sorted_index_dict = sorted(index_dict.items(), key=lambda x: x[1], reverse=False)
        pred_list = [sorted_index_dict[i][0] for i in range(len(sorted_index_dict))]
        final_2stage_pre_dict.update({i:pred_list})

    return final_2stage_pre_dict
